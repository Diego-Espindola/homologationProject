...
class App(tk.Tk):
    def __init__(self, email_remetente, email_passwd):
        super().__init__()

        ## Informações do email
        self.servidor_smtp = 'mailserver.bethacloud.com.br'
        self.porta_smtp = 465
        self.email_remetente = email_remetente
        self.senha_remetente = email_passwd
    ...
    def validar_token(self):
        ...
        if not self.token.lower().startswith("bearer"):
            self.token = f"bearer {self.token.rstrip()}"
        return True
