...
def checar_discrepancias_matricula(rows, buscaMatricula, execucaoPrincipal):
    ...
    eh_conselheiro = 'S' if matricula["conselheiroTutelar"] else 'N'
    ...
    if rows.dataBase != matricula['dataBase']:
...
def checar_discrepancias_endereco(desktop_data, cloud_data, execucaoPrincipal):
    ...
    nao_migrados = pd.merge(esquerda, direita, on=['logradouro.id', 'bairro.id'], how='left', indicator=True)
    nao_migrados = nao_migrados[nao_migrados['_merge'] == 'left_only'].drop('_merge', axis=1)
    if not nao_migrados.empty:
        for index, row in nao_migrados.iterrows():
            gerenciadorErros.descricao = ""
            desktop_data = row[['complemento', 'numero', 'cep']].to_dict()
            execucaoPrincipal.inserir_erro(tipo_erro="PESSOA_ENDERECO_NAO_ENCONTRADO", gerenciadorErros=gerenciadorErros, desktop=desktop_data, cloud="")
...
def checar_discrepancias_dependentes(desktop_data, cloud_data, execucaoPrincipal):
    ...
    if not nao_migrados.empty:
        for index, row in nao_migrados.iterrows():
            ...
            desktop_data = "\n".join([f"{key}:{value}" for key, value in desktop_data.items()])
    ...
    if not migrados_extra.empty:
        for index, row in migrados_extra.iterrows():
            ...
            cloud_data = "\n".join([f"{key}:{value}" for key, value in cloud_data.items()])
