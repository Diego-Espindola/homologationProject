...
def processar_enderecos(app, pessoas_fisicas, progress_bar, label):
    ...
    for pessoa in pessoas_fisicas:
        ...
        if pessoa['historico_pessoa']['enderecos']:
            df_cloud_endereco = pd.json_normalize(pessoa['historico_pessoa']['enderecos'])
        else:
            df_cloud_endereco = pd.DataFrame(
                columns=['id', 'cep', 'numero', 'complemento', 'descricao', 'principal', 'logradouro.id', 'bairro.id']
            )
        try:
            ...
            print(f"erro endereços: {e}\nDESK{df_desktop_endereco}\nCLOUD{df_cloud_endereco}")
